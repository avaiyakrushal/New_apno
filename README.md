# Apno Android prototype (version 0.4)

This source project has a white five-tab home screen modeled on the supplied layout references. Party offers local posts and room previews. Discover lists those local rooms. Game has a working single-player dice roll; other game cards are explicit placeholders. Messages shows private notes stored on this phone; online conversations await account and chat integration. Profile has editable name, bio and gallery photo, a local party room list, and working bottom navigation.
